    ...
    <div class="findShop-top pb10 ">
      <van-row class="bxs pr10">
        <van-col span="20" class="pt10 pr bxb pr20">
          <van-icon name="arrow-left"  size='1.8rem'  @click="backHome" class="findShop-back"/>
          <input style="color: black" class="top-search ml15 pl15 bxb" v-model="search" placeholder="请输入"/>
          <van-icon name="search" class="search-icon fb" style="top: 15px;color: #FFBB00;" @click="suponSearch" />
        </van-col>
      </van-row>
    </div>
    ...
    <van-sticky :offset-top="0">
      <div class="mt20 pt40 pb10" style="background:white;">
        <van-row class="ml20 mr5" >
          <van-col span="5" class="active-font-weight"><span class="pr"><span class="kailong-up kuicon"></span></span>转让</van-col>
...
<script>
  ...
  import Vue from 'vue'
  import {Toast,Tab, Tabs ,Image as VanImage ,Tag,Divider,Lazyload,Loading , Tabbar, TabbarItem,Search,DropdownMenu, DropdownItem,Popup,Checkbox, CheckboxGroup, Sticky } from 'vant';
  Vue.use(Tab).use(Tabs).use(VanImage).use(Tag).use(Divider).use(Loading).use(Tabbar).use(TabbarItem).use(Search).use(DropdownMenu).use(DropdownItem).use(Popup).use(Checkbox).use(CheckboxGroup);
  Vue.use(Lazyload, {
    lazyComponent: true,
  }).use(Sticky);
  import {getRentoutListPageIndex,getLocalCityList, getRentoutListRPage,getSellListSPage, getRentoutListPage, getCListRPage, getAlListPage, getSellListPage } from '@/api/rentout'
  export default {
    name: 'findShop',
    data() {
      return {
    ...
    methods: {
      ...
      getRentoutListPage() {
        var data2 = localStorage.getItem('city');//从本地存储取出
        getRentoutListPageIndex({search: this.search,current: this.currentPage, size: this.pagesize, county: this.county,
          monthlyRentS: this.monthlyRentS, monthlyRentE: this.monthlyRentE,
          acreageS: this.acreageS, acreageE: this.acreageE,
          mating: this.mating, industry: this.industry,type:3,shopType: this.shopType,city: data2}).then(response => {
