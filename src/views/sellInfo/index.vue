      ...
      <div class="fh" v-if="show" style="width: 100%;">
        <div style="background: #f2f2f2">
          ...
          <van-swipe :autoplay="3000" @change="onChange" class="pr" style="height: 200px" >
            <van-swipe-item v-for="(item,index) in imgList" :key="index" @click="picBanner(index)">
              ...
              <div v-if="item.url!=null">
                <van-image  :src="item.url" fit="fill" width="100%" height="200px"  lazy-load />
              </div>
              <div v-else>
                <van-image fit="fill"  src="../../../static/images/logo.png" width="100%" height="200px"    lazy-load />
              </div>
            </van-swipe-item>
          ...
          <div class="pt20 pb15 bgf fh container">
            ...
            <van-row>
              <van-col span="8">
                <p> <span style="font-family: 'Arial Black'" class="f18 cred fb">
                  <span v-if="rentoutForm.totalPrice > 10000">{{rentoutForm.totalPrice/10000}}万</span>
                   <span v-if="rentoutForm.totalPrice < 10000">{{rentoutForm.totalPrice}}万</span>
                </span>
                </p>
                <p>  <span  class="f14 ">售价 </span></p>
              </van-col>
              <van-col span="8">
          ...
          <div class="c666 mt10 pb10 bgf fh container">
            ...
            <van-row class="c666  mt10">
              ...
              <van-col  span="19" >
                  <span  v-for="item in rentoutForm.passengerFlowList">
            ...
            <van-row class="c666 mt10">
              ...
              <van-col  span="19" >
                  <span  v-for="item in mating" v-if="item">
                    <van-tag v-if="item!=''" class="ml5" type="primary" >{{item}}</van-tag>
                  </span>
              </van-col>
            </van-row>
          ...
          <!--          <div class="pb15 bgf fh container">-->
          <!--            <img v-for="(item,index) in imgList" :key="index" :src="item.url" class="bk mt10" width="100%" >-->
          <!--          </div>-->
          ...
          <div class=" bgf fh van-hairline--bottom">
            <!--            <div class="pt10 f18 pb10 fb cred">周边分析</div>-->
            ...
            <div id="allmap" style="height: 350px;margin-bottom: 15px;margin-top: 10px"></div>
            <!--            <div id="myChart" style="width: 100%;height:400px"></div>-->
            <!--            <div id="bar" style="width: 100%;height:400px"></div>-->
          </div>
          <div class="mt15 pb15 bgf fh container">
              ...
              <van-row class="">
                <van-col :span="24">
                  <div v-if="rDateList.length > 0">
                    <div class="item" v-for="item in rDateList" :key="item.id" @click="into(item.id)">
                      <van-divider />
                        <van-row>
                          <van-col span="9">
                            <div v-if="item.url!=null">
                              <van-image  :src="item.url" fit="fill" width="100%" height="120px" radius="8px"  lazy-load />
                            </div>
                            <van-image v-else  src="../../../static/images/background.jpg" fit="fill" width="100%" height="120px"  radius="8px"lazy-load />
                          </van-col>
                          <van-col span="15">
...
<script>
  ...
  import Vue from 'vue'
  import {Image as VanImage ,Tag,Divider, Lazyload ,ImagePreview , Swipe,Toast,ShareSheet, SwipeItem, NavBar} from 'vant';
  Vue.use(VanImage).use(Tag).use(Divider).use(Lazyload).use(ImagePreview).use(Swipe).use(Toast).use(ShareSheet).use(SwipeItem).use(NavBar);
  Vue.use(Lazyload, {
    lazyComponent: true,
  });
  import { getSellInfoById, getSellListPage,saveViewNum } from '@/api/rentout'
  export default {
    name: 'transferInfo',
    data() {
      return {
        ...
        config: {
          url:{
            left: '../../../static/images/right.jpg',
            front: '../../../static/images/front.jpg',
            right: '../../../static/images/back.jpg',
            back: '../../../static/images/left.jpg',
    ...
    methods: {
      ...
      getSellListSPage() {
       var data = localStorage.getItem('city');//从本地存储取出
        getSellListPage({ current: 1, size: 5,type:1 ,city:data }).then(response => {
      ...
      getrentoutInfo() {
        var _this = this
        getSellInfoById({ sellId: this.rentoutId }).then(response => {
      ...
      //   // 绘制图表
      //   myChart.setOption({
      //     title: { text: '本店周边各行业商铺数量汇总图' ,  subtext: '附近1公里', x:'center'},
      //     tooltip: {},
      //     xAxis: {
      //       data: this.key1
      //     },
      ...
      //               '#FE8463','#9BCA63','#F4E001','#D7504B','#26C0C0'
      //             ];
      ...
      startMap(adr){
        ...
        localSearch.setSearchCompleteCallback(function (searchResult) {
          ...
          that.searchData(that.key1,function (res) {
            // var data = [];
            // for(var i = 0,dd;dd=res[i++];){
            //   data.push(dd.Br.length)
            // }
            // that.drawLine(data)
          })

          // that.searchDataNo(that.key2,function (res) {
          //   console.log(res)
          //
          //   var data = [];
          //   for(var i = 0,dd;dd=res[i++];){
          //     data.push({name:dd.keyword,value:dd.Br.length})
          //   }
          //   that.drawBar(data)
          // })
