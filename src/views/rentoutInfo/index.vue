...
<!--                <img v-lazy="item.url" width="100%" height="100%">-->
                    ...
                    <div v-for="item in rDateList" :key="item.id" @click="into(item.id)" >
                      <van-row>
                        <van-col span="9">
                          ...
                          <!--                        <img class="" :src="item.url" height="140px" width="100%" />-->
                        </van-col>
                        <van-col span="15">
                          <div class="ml10 " style="font-weight: bolder">
                            ...
                            <van-row class="mt10" v-if="item.type===3">
...
<script>
  ...
  import Vue from 'vue'
  import {Image as VanImage ,Tag,Divider, Toast,ShareSheet, Lazyload ,ImagePreview , Swipe, SwipeItem,NavBar} from 'vant';
  Vue.use(VanImage).use(Tag).use(Divider).use(Toast).use(ShareSheet).use(Lazyload).use(ImagePreview).use(Swipe).use(SwipeItem).use(NavBar);
  Vue.use(Lazyload, {
    lazyComponent: true,
  });
  ...
  export default {
    ...
    data() {
      return {
        ...
        bannerlength:0,
        bannerlist:[],
        imagetype:1, //1=图片 2=全景图 3=效果图 4=实景图 5=户型
        index: 0,
        show:false,
        transitionName:'',
        current: 0,
    ...
    methods: {
      ...
      getrentoutInfo() {
        ...
        getRentoutInfoById({rentoutId: this.rentoutId}).then(response => {
          ...
          // this.type= _this.rentoutForm.type
      ...
      getRentoutListRPage() {
       var data= localStorage.getItem('city');
        getRentoutListPage({current: 1, size: 8, type: 1,city: data}).then(response => {
      ...
      searchData(keywards, onSearchComplete = function () {
        ...
        var circle = new BMap.Circle(this.mPoint, 400, {
          fillColor: "blue",
          strokeWeight: 1,
          fillOpacity: 0.3,
          strokeOpacity: 0.3
