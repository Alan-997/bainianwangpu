<template>
  <div id="businessCircle" style="width: 100%;overflow: hidden">
   <van-sticky :offset-top="0">
    ...
    <div class="van-hairline--top-bottom tab-position" style="background: white;">
      <van-tabs @click="onClick" class=" ">
        <van-tab  v-for="(item,index) in listTabs" :title="item.name" :key="index" >
          <!--商圈数据列表-->
          <div class="van-hairline--bottom" style="float: none" >
                <van-row class="">
                  <van-col :span="24">
                    <div v-if="businessList.length > 0" >
                      <router-link v-for="item in businessList" :key="item.id" :to="pingUrl(item.id)" class="">
                        <van-row >
                          <van-col span="9">
                          ...
                          <van-col span="13" >
                            <div class="ml10">
                              <p  class="c13 f16 tf2 van-ellipsis">{{item.title}}</p>
                              <div class="">
                                <van-row class="mt5">
                                  <van-col span="8"  v-if="item.price>0">
                                    <span class="f14 fb" style="color: red" >￥{{item.price}}</span>
                                ...
                                <van-row class="mt5">
                                    ...
                                    <van-col class="ml5 mr5" >|</van-col>
                                    <van-col  class="f12 c666 t-left van-ellipsis">{{item.addr}}附近</van-col>
                                </van-row>
...
<script>
  ...
  import Vue from 'vue'
  import {  Tag,Image as VanImage, Divider , Toast, Tab, Tabs  ,Lazyload, PullRefresh, Tabbar, list,TabbarItem,Search,DropdownMenu, DropdownItem,Swipe, SwipeItem,Popup,Checkbox, CheckboxGroup,Sticky } from 'vant';
  Vue.use(Tag).use(VanImage).use(Divider).use(Toast).use(Tab).use(Tabs).use(PullRefresh).use(Tabbar).use(TabbarItem).use(list).use(Search).use(DropdownMenu).use(DropdownItem).use(Swipe).use(SwipeItem).use(Popup).use(Checkbox).use(CheckboxGroup);
  Vue.use(Lazyload, {
    lazyComponent: true,
  }).use(Sticky);
     ...
     onClick(name, title) {
      ...
      getAlListPage() {
        ...
        getBusiness({ current: this.pageIndex, size: this.pageSize, search: this.search,
          ...
          cityCode: this.cityCode, areaCode:this.areaCode, jd: this.jd, wd: this.wd,
        }).then(response => {
